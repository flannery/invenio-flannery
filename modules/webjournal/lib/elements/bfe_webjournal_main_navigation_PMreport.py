...
def format(bfo, category_prefix, category_suffix, separator=" | ",
    ...
    linkattrd = {}
    if selected_category.lower() == "":
        linkattrd = {'class':'selectedNavigationPage'}
    meetings = create_html_link( CFG_SITE_URL + "/journal/meetings?name=%s&issue_year=%s&issue_number=%02d" % ( journal_name, this_issue_number.split("/")[1], int( this_issue_number.split("/")[0] )),  {},  _('Meetings'), linkattrd=linkattrd )
    categories_links.append(meetings)
