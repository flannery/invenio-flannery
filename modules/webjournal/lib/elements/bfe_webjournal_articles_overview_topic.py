...
#            subtopics = topics.findall( "category/subcategory" )
#            try:
#                topic_title = [topic.get('title') for topic in \
#                                 subtopics \
#                                 if topic.get('name').lower() == article_topic.lower()]
#            except:
#                return "Could not find category title for : %s" % article_topic
            ...
            if text != '':
                if i == 0:
                    out += '''
                            ...
                            <h%(header_tag_size)s class="%(css_classes)s articleTitle" style="clear:both;">
                                <span class="%(topic_class)s">%(topic_title)s </span><a title="link to the article" href="%(article_link)s">%(title)s</a>
                            </h%(header_tag_size)s>
                    ...
                    i = i + 1
                    if column == 1:
                        out += '<div id="article_columns"><ul id="article_list">'
                    else:
                        out += '<ul>'  
                else:
                    if (column == 1) & (i % 2 == 1):
                        out += '<li class="article left">'
                    else:
                        out += '<li class="article">'
                    out += '''
                            <h%(header_tag_size)s class="%(css_classes)s articleTitle" style="clear:both;">
                                <span class="%(topic_class)s">%(topic_title)s </span><a title="link to the article" href="%(article_link)s">%(title)s</a>
                            </h%(header_tag_size)s>
                ...
                out += '''
                       ...
                       ''' % {'article_link': article_link,
                            ...
                            'topic_title':topic_title}
    if column == 1:
        out += '<div style="clear: both"></div></ul></div>'
    else:
        out += '</ul>'
